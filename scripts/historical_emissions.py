...
# downloaded 20200525 (modified by EEA last on 20200529)
...
cts = ["CH","NO"] + eu28_eea 
cts=['ES']
year = np.arange(1990,2019).tolist()
#%%
idx = pd.IndexSlice
...
emissions = (1/1e3)*emissions.groupby(level=0, axis=0).sum() #Mton CO2
#%%
...
#ax1.plot(np.arange(1990,2018), emissions.loc['LULUCF'], 
#                      color='gold',
#                      linewidth=4,
#                      label='LULUCF') 
ax1.stackplot(np.arange(1990,2019), [pd.to_numeric(emissions.loc['LULUCF']),], 
                      colors=['gold'],
                      linewidth=0,
                      labels=['UTCUTS']) 
    
ax1.stackplot(np.arange(1990,2019), [                      
...
#                              'electricity generation + CHP',])
                     labels=[ 'trasporte carretera', 'trasporte ferrocarril', 
                              'marítimo doméstico', 'aviación doméstico',
                              'marítimo interacional', 'aviación internacional',
                              'industria (procesos)', 'industria (energia)', 
                              'agricultura', 'residuos', 'energía (otros)',
...
#ax1.set_ylabel('GHG emissions ( CO$_{2,eq}$ Mt per year)',fontsize=22)
ax1.set_ylabel('Emisiones GEI (Mt CO$_{2,eq}$ / año)',fontsize=22)
ax1.set_xlim([1990, 2032])
#ax1.set_ylim([0,2.5])
...
ax1.plot([2030],[0.77*total_1990],
         marker='*', markersize=12, markerfacecolor='black',
         markeredgecolor='black', label='objetivo -23% (PNIEC)',
         linewidth=0)
...
ax1.legend(reversed(handles), reversed(labels), fancybox=True, fontsize=20, 
               shadow=True, loc=(1.02,0.05), facecolor='white', frameon=True)
        
# ax1.legend(fancybox=True, fontsize=20, shadow=True, 
#            loc=(1.05,0.05), facecolor='white', frameon=True)

plt.tight_layout()
ax1.set_title('España', fontsize=22)
plt.savefig('figures/emisiones_historicas_sectores.png', dpi=300, bbox_inches='tight')
